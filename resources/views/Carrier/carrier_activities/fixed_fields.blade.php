    ...
    <div class="form-group col-sm-6">
        {!! Form::label('rebate_financial_bonuses_step_rate_json','存送固定红利').'<span style="color:red">(可设置多个条件)</span>' !!}
    </div>
    <div id="fixedFlowSetting">
        <input type="hidden" name="fixed_json" v-model="resultJson">
...
<script>
    $(function(){
        var fixedFlowSetting = new Vue({
            el:'#fixedFlowSetting',
            data:{
                displayedSettingRows:[]
            },
            created:function(){
                @if(isset($carrierActivity) && $carrierActivity->bonuses_type ==2 && $rebate_financial_flow_step_rate_json = $carrierActivity->rebate_financial_bonuses_step_rate_json )
