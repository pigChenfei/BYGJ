<div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
        ...
        {!! Form::model($playerWithdrawLog, ['route' => ['playerWithdrawLogs.update', $playerWithdrawLog->id], 'method' => 'patch' , 'id' => 'playerWithDrawReviewForm']) !!}
        <div class="modal-body" id="modalContent">
            <div class="form-group col-sm-12">
                {!! Form::label('', '取款信息') !!}
                <dl class="dl-horizontal">
                    <dt>持卡人</dt>
                    <dd>{!! $playerWithdrawLog->bankCard->card_owner_name !!}</dd>
                    <dt>开户行</dt>
                    <dd>{!! $playerWithdrawLog->bankCard->bankType->bank_name !!}</dd>
                    <dt>卡号</dt>
                    <dd>{!! $playerWithdrawLog->bankCard->card_account !!}</dd>
                    <dt>分行</dt>
                    <dd>{!! $playerWithdrawLog->bankCard->card_birth_place !!}</dd>
                </dl>
            </div>
            <div class="form-group col-sm-12">
                <label for="">今日取款次数: {!! $playerTodayWithdrawCount !!}次</label>
            </div>
            <div class="form-group col-sm-12">
                {!! Form::label('', '出款申请金额') !!}
                <input type="text" readonly class="form-control" value="{!! $playerWithdrawLog->apply_amount !!}">
            </div>
            <div class="form-group col-sm-12">
                {!! Form::label('', '选择出款银行') !!}
                <select name="carrier_pay_channel" style="width: 100%" class="form-control disable_search_select2">
                    @foreach($banks as $payChannel)
            ...
            <div class="form-group col-sm-12">
                ...
                <input id="fee_amount_input" type="number" name="fee_amount" min="0" max="{!! $playerWithdrawLog->apply_amount !!}" v-model="fee" class="form-control">
            </div>
            <div class="form-group col-sm-12">
            ...
            <input type="hidden" name="status" value="{!! \App\Models\Log\PlayerWithdrawLog::STATUS_PAYED_OUT !!}">
            <div class="clearfix">
            </div>
        </div>
        <div class="modal-footer">
            <div class="form-group col-sm-12">
                {!! TableScript::editFormSubmitAndCancelButtonsScript(route('playerWithdrawLogs.update',$playerWithdrawLog->id)) !!}
            </div>
        </div>
        {!! Form::close() !!}
    </div>
...
<script>
    $(function(){
        ...
        new Vue({
            ...
            data:{
                applyWithDrawAmount : '{!! $playerWithdrawLog->apply_amount !!}',
                fee: 0,
                feeBearType:[
                    {text:'公司',value:'company'},//{text:'代理',value:'agent'},
                    {text:'会员',value:'player'}
                ],
