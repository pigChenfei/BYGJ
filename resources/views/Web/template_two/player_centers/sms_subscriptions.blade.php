                ...
                <div class="art-body">
                                ...
                                @if($stationLetterList->items())
                                    @foreach($stationLetterList as $v)
                                        <tr class="sms-list" id="mes_{!! $v->id !!}">
                                            ...
                                            <td class="theme" data-value="{!! $v->id !!}"><i>{!! $v->carrierPlayerNews->title !!}</i></td>
                                            <td>{!! $v->created_at !!}</td>
                                            <td>{!! empty($v->player_view_status)?'未读':'已读' !!}</td>
                                        </tr>
                                    @endforeach
                                @endif
                            </tbody>
                    ...
                    @else
                        ...
                        <div class="pagenation-container clearfix">
                            ...
                            {{ $stationLetterList->links('Web.'.\WinwinAuth::currentWebCarrier()->template.'.pageStyle.template_one', 1) }}
                        </div>
                    @endif
                </div>
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(data.success == true){
                            ...
                            $('.masklayer').show();
                            $('.masklayer').find('.dialog-head').html(data.data.carrier_player_news.title);
                            $('.masklayer').find('.dialog-remark').html(data.data.carrier_player_news.remark);
                        }
                    },
                    error: function(xhr){
                        if(xhr.responseJSON){
