                ...
                <div class="art-body">
                    ...
                    <div class="memb-box clearfix">
                        ...
                        <div class="form-inline float-left" style="margin-bottom: 20px;margin-top:10px;">
                            <div class="draw-item"><span>可取金额：</span><i class="font-red can-draw">{!! str_limit(number_format(\WinwinAuth::memberUser()->main_account_amount,2),14) !!}</i></div>
                            <div class="draw-item"><span>完成流水：</span><i class="font-red drawed">{!! number_format($complete, 2) !!}</i></div>
                            <div class="draw-item"><span>未完成流水：</span><i class="font-red undraw">{!! number_format($unfinished, 2) !!}</i></div>
                        </div>
                        <div class="float-right">
                            <div class="form-inline mb-10 draw-money-info">
                                <label class="sm" for="draw-money-num">取款金额：</label>
                                <input type="text" autocomplate="off" class="form-control" placeholder="单次最低提款额为{!! $carrierWithdrawConf->player_once_withdraw_min_sum??0 !!}元,最高{!! $carrierWithdrawConf->player_once_withdraw_max_sum??0 !!}元" id="draw-money-num" data-min="{!! $carrierWithdrawConf->player_once_withdraw_min_sum !!}" data-max="{!! $carrierWithdrawConf->player_once_withdraw_max_sum !!}"/>
                            </div>
                        ...
                        <div class="msg font-red float-right text-center" style="width:435px;padding-top: 10px;"></div>
                    </div>
            ...
            $(".submit_button").on('click',function(e) {
                ...
                if (card_id == "") { //没有绑定银行卡
                    ...
                    $.ajax({
                        ...
                        error: function(xhr){
                            if(xhr.responseJSON){
                                button.parent().prev().find('div.msg.font-red.text-center').html(xhr.responseJSON.message);
                            }
