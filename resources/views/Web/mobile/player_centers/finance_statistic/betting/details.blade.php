    ...
    <div class="page-group">
      <div class="page page-current" id="page-tablist">
        ...
        <header class="bar bar-nav"><a class="icon icon-left back"></a>
          <h1 class="title">投注详情</h1>
        </header>
        <!--内容区-->
        <div class="content native-scroll">
          <div class="time-wrap text-center mt10"><span class="time-begin">{!! $data['betting_detail_start'] !!}</span>-<span class="time-end">{!! $data['betting_detail_end'] !!}</span></div>
    	...
    	$(document).on("pageInit", "#page-tablist", function(e, id, page) {
    		...
    		$(page).on('infinite', function() {
    			...
    			setTimeout(function() {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "{!! route('players.bettingDetails') !!}",
                        data: {
                            'gamePlatId' : "{!! $data['gamePlatId'] !!}",
                            'page' : spage
                        },
                        dataType: 'json',
                        success: function (xhr) {
                            var tempdata =xhr.data;
