    ...
    <div class="page-group">
      <div class="page page-current" id="page-tablist">
        ...
        <header class="bar bar-nav"><a class="icon icon-left back"></a>
          <h1 class="title">@if($tabletype==1) 统计概况 @else 结算详情 @endif</h1>
        </header>
        <!--内容区-->
        <div class="content native-scroll">
          <div class="time-wrap text-center mt10"><span class="time-begin">{!! $starttime !!}</span>-<span class="time-end">{!! $endtime !!}</span></div>
    	...
    	if(data && data.length!=0){
    		data.forEach(function(item, index){
    			if(typeof item != 'object'){
    				...
    				var $wrap = $("<div class='list-block  clearfix'/>")
    				for( var key in item ){
    					var val = item[key]
