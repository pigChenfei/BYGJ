 ...
 * @property string $template_agent_admin 邮箱
 * @property string $promotion_url 代理推广网址
 * @property int $promotion_url_click_number 代理推广网址点击次数
 * @property string $promotion_notion 代理邀请介绍
 * @property string $promotion_code 邀请码
...
class CarrierAgentUser extends Auth
{
    ...
    {
        if ($this->status == 0) {
            throw new AgentAccountException('代理商账户被禁用或正在审核');
        }
        return true;
    }
    ...
    {
        return AgentBearUndertakenLog::isSettled(false)->undertakeType(AgentBearUndertakenLog::UNDERTAKEN_TYPE_BET_FINANCIAL_FLOW)
            ->byAgentUser($this->id)
            ->sum('amount');
    }
     ...
     * 代理取款银行卡信息
     */
    public function agentSettle()
    {
        return $this->hasMany(CarrierAgentSettleLog::class, 'agent_id', 'id');
    }
    ...
    public function getAllUpAgents(CarrierAgentUser $agent, $dep = 5)
    {
        static $upAgents = array();
        static $i = 0;
        $upAgent = $agent->parentAgent;
        if ($agent->parent_id != $agent->id && $agent->parent_id > 0 && ! is_null($upAgent) && $upAgent->agent_level_id == $agent->agent_level_id && $upAgent->is_default == 0 &&
             $i <= ($dep - 1)) {
            $upAgents[$i] = $upAgent;
            $i ++;
            return $this->getAllUpAgents($upAgent);
        }
        return $upAgents;
    }
}
