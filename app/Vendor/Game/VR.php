...
class VR extends GameAbstract
{
    ...
    public function checkTransfer(PlayerTransfer $playTransfer, PlayerGameAccount $account)
    {
        $url = self::URL . self::TRANSACTIONRECORD;
        $data = array(
            'serialNumber' => $playTransfer->transid,
            'startTime' => gmdate("Y-m-d\TH:i:s\Z", strtotime($playTransfer->created_at) - 12 * 3600),
            'endTime' => gmdate("Y-m-d\TH:i:s\Z", strtotime($playTransfer->created_at) + 12 * 3600),
            'playerName' => $account->account_user_name,
            'recordPage' => 0,
        ...
        $transfer = $this->vrCurl($url, $body);
        if ($transfer && $transfer['data'] && count($transfer['data']) > 0) {
            $tranData = $transfer['data'][0];
            if (($tranData['type'] + 1) == $playTransfer->direction && bccomp($tranData['amount'], $playTransfer->money, 2) == 0) {
                if ($tranData['state'] == 0) {
                    return 1;
                }
                return - 1;
