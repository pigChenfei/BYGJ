...
class MG extends GameAbstract
{
    ...
    {
        $url = self::HOST . self::API_TRANSACTION . '?account_ext_ref=' . $account->account_user_name . '&ext_ref=' . $playTransfer->transid;
    ...
    public function transferIn($accountUserName, $money)
    {
        ...
        $output = $this->mgCurl($url, json_encode(array(
            $param
        )));
        if (! $output) {
            return array(
    ...
    public function createMember($pre)
    {
        ...
        $param = array(
            ...
            'password' => $this->generate_value(8)
        );
        $output = $this->mgCurl($url, json_encode($param));
