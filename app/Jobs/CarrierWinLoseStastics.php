...
//        $bonus = PlayerDepositPayLog::payedSuccessfully()->whereBetween('created_at',[$startTimeString,$endTimeString])->where('carrier_id',$this->carrier->id)->sum('bonus_amount');
        $bonus = PlayerAccountLog::whereBetween('created_at',[$startTimeString,$endTimeString])->where('fund_type',PlayerAccountLog::FUND_TYPE_BONUS)->where('carrier_id',$this->carrier->id)->sum('amount');
