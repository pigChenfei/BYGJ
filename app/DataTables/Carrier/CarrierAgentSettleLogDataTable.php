...
class CarrierAgentSettleLogDataTable extends DataTable
{
    ...
    public function query()
    {
        $carrierActivityAudit = CarrierAgentSettleLog::with([
            'agent.agentLevel',
            'settlePeriods'
        ])->where('status', '!=', CarrierAgentSettleLog::SET_COMPLETED_STATUS)->orderBy('updated_at', 'desc');
            ...
            [
                ...
                $(".user_edit").on("click",function(){
                    ...
                    $.fn.winwinAjax.buttonActionSendAjax(_me,"' .
                     route('carrierAgentSettleLogs.gamePlatWinAmount', null) . '/"+ user_id,{},function(content){
                        $.fn.overlayToggle();
                ...
                $(".manual_tuneup_edit").on("click",function(){
                    ...
                    var user_id = $(_me).attr("data-id")
                    $.fn.winwinAjax.buttonActionSendAjax(_me,"' .
                     route('carrierAgentSettleLogs.manualTuneup', null) . '/"+ user_id,{},function(content){
                        $.fn.overlayToggle();
                        $("#userInfoEditModal").html(content);
                        $("#userInfoEditModal").modal("show");
                    },function(){
                ...
                $(".actual_payment_edit").on("click",function(){
                    ...
                    var user_id = $(_me).attr("data-id")
                    $.fn.winwinAjax.buttonActionSendAjax(_me,"' .
                     route('carrierAgentSettleLogs.actualPayment', null) . '/"+ user_id,{},function(content){
                        $.fn.overlayToggle();
                        $("#userInfoEditModal").html(content);
                        $("#userInfoEditModal").modal("show");
                    },function(){
    ...
    private function getColumns()
    {
        return [
            ...
            '手工调整' => [
                ...
                'orderable' => false,
                'render' => 'function(){
                      return "<a data-toggle=\"tooltip\" data-original-title=\"手工调整佣金："+this.manual_tuneup+"  手工调整洗码金额："+this.manual_tuneup_rebate+"\" class=\"text-primary manual_tuneup_edit\" data-id=\""+ (this.id ? this.id : null) +"\" style=\"cursor: pointer\">" + (this.manual_tuneup_all ? this.manual_tuneup_all : 0.00) + "</a>"
            }'
            ],
            '本期佣金' => [
            ...
            '实际发放' => [
                ...
                'orderable' => false,
                'render' => 'function(){
                      return "<a data-toggle=\"tooltip\" data-original-title=\"实际发放佣金："+this.actual_payment+"  手工调整洗码金额："+this.actual_payment_rebate+"\" class=\"text-primary actual_payment_edit\" data-id=\""+ (this.id ? this.id : null) +"\" style=\"cursor: pointer\">" + (this.actual_payment_all ? this.actual_payment_all : 0.00) + "</a>"
            }'
            ],
            '结转下月' => [
