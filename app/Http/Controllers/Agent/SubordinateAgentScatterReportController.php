...
use App\DataTables\Agent\SubordinateAgentScatterReportDataTable;
...
class SubordinateAgentScatterReportController extends AppBaseController
{
     ...
     * @return Response
     */
    public function index(SubordinateAgentScatterReportDataTable $subordinateAgentScatterReportDataTable)
    {
        return $subordinateAgentScatterReportDataTable->render('Agent.subordinate_agent_scatter_reports.index');
    }
    ...
    {
        $agentSettlePeriodsLog = CarrierAgentSettlePeriodsLog::where('agent_id',$id)->first();
        ...
        $betFlowSetting  = CarrierAgentUser::with('agentLevel.rebateFinancialFlowAgentGamePlatConf')->where('id',$agentSettlePeriodsLog->agent_id)->first();
        //根据代理ID找出所有的所属玩家的ID
        $agentMember = Player::where('agent_id',$agentSettlePeriodsLog->agent_id)->get();
        foreach ($agentMember as $item){
            $agentMemberID[] = $item->player_id;
        }

        //子代理会员
        $subAgentSettlePeriodsLog = CarrierAgentSettlePeriodsLog::where('agent_id',$agentSettlePeriodsLog->agent_id)->first();
        ...
        //代理基本设置
        $agentBaseInfoConf = CarrierAgentUser::where('id',$agentSettlePeriodsLog->agent_id)->with('agentLevel.rebateFinancialFlowAgentBaseConf')->first();
...
//       $effectivePlayer = PlayerBetFlowLog::whereIn('player_id',$agentMemberID)->get();
        ...
        return view('Agent.subordinate_agent_scatter_reports.details')->with(['gamePlatBetFlow'=>$gamePlatBetFlow,'gamePlatName'=>$gamePlatName,'betFlowSetting'=> $betFlowSetting,'agentStatistics' => $agentStatistics]);
    }
